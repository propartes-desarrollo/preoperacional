...
router.get('/', async (req, res, next) => {
  try {
    const { rows } = await pool.query(
      `SELECT t.id, t.name, t.uses_company_vehicles, t.requires_inspection, t.is_active, t.display_order,
...
router.post('/', async (req, res, next) => {
  try {
    const { name, uses_company_vehicles, requires_inspection, display_order } = req.body;
    if (!name || !name.trim()) {
    ...
    const { rows } = await pool.query(
      `INSERT INTO collaborator_types (name, uses_company_vehicles, requires_inspection, display_order)
       VALUES ($1, $2, $3, $4) RETURNING *`,
      [name.trim(), uses_company_vehicles ?? false, requires_inspection ?? false, display_order ?? max_order + 1]
    );
...
router.put('/:id', async (req, res, next) => {
  try {
    const { name, uses_company_vehicles, requires_inspection, is_active, display_order } = req.body;
    const { rows } = await pool.query(
      `UPDATE collaborator_types SET
        name = COALESCE($1, name),
        uses_company_vehicles = COALESCE($2, uses_company_vehicles),
        requires_inspection = COALESCE($3, requires_inspection),
        is_active = COALESCE($4, is_active),
        display_order = COALESCE($5, display_order)
       WHERE id = $6 RETURNING *`,
      [name?.trim() ?? null, uses_company_vehicles, requires_inspection, is_active, display_order, req.params.id]
    );
